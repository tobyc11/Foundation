...
public:
    ...
    unsigned int AddRef() const { return RefCount.fetch_add(1, std::memory_order_relaxed) + 1; }
...
template <typename T> class TAutoPtr
{
public:
    // Null initialization
    TAutoPtr() noexcept
    ...
    explicit TAutoPtr(std::nullptr_t)
        : Ptr(nullptr)
    {
    }

    TAutoPtr& operator=(std::nullptr_t)
    {
    ...
    // Copy
    explicit TAutoPtr(T* ptr, bool addRef = true)
        : Ptr(ptr)
    {
        if (addRef)
    ...
    // Dtor
    ~TAutoPtr() { MACRO_SAFE_RELEASE_NULL(Ptr); }
    ...
    void Clear() { MACRO_SAFE_RELEASE_NULL(Ptr); }

private:
    T* Ptr;
};

template <class T> inline bool operator<(const TAutoPtr<T>& r, std::nullptr_t)
{
    return std::less<T*>()(r.Get(), nullptr);
}

template <class T> inline bool operator<(std::nullptr_t, const TAutoPtr<T>& r)
{
